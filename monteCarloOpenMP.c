...
int main(int argc, char** argv)
{
    ...
    long int nIterations = 1000000000;
    /* initialize random numbers */
    srand(SEED);
    count=0;

    int numthreads=8;
	#pragma omp parallel private(x, y, z, i) reduction(+:count) num_threads(numthreads)
    {
    ...
    printf("count is %ld \n", count);
    printf("# of trials= %ld , Parallel estimate of pi is %6.6f \n",nIterations,pi);
